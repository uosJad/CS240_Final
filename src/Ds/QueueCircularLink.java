...
public final class QueueCircularLink<T>  implements QueueInterface<T>{
	...
	public void enqueue(T newEntry) {
		...
		Node temp = getBackNode(); //node that points to back
		
		// if free is point to self or back, create new node
		if (temp == free && numOfItems + 1 <= MAX_SIZE){
			temp = new Node(newEntry, back); //point to free
			back = temp;
			free.adr = back; // free point back
			numOfItems++;
			//System.out.print("new node1:");
		}		
		// else, if existing null node that is not free, reuse existing
		// if node is pointing to back but is not the free node, then it is empty
	...
	public T dequeue() {
		...
		if(!isEmpty()){
			...
			free = getFrontNode(); // move free up one, free's space for reuse
			numOfItems--;
		}
